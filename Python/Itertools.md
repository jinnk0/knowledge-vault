...
파이썬에서 반복자란 [[Python/반복문.md|for 반복문]]과 함께 사용할 수 있는 모든 타입을 뜻하며, [[Python/컬렉션/리스트.md|리스트]], [[Python/컬렉션/튜플.md|튜플]], [[Python/컬렉션/딕셔너리.md|딕셔너리]], [[Python/컬렉션/Set.md|Set]] 등이 내장 반복자의 예시다. 반복자 객체가 반드시 소진(종료)되어야 하는 것은 아니며 때로는 무한할 수도 있는데, 이런 반복자를 무한 반복자라 한다. 파이썬은 세 가지 무한 반복자를 제공한다.
...
순열, 조합, 데카르트 곱(Cartesian product)과 같은 조합론적 구조를 단순화하는 데 쓰이는 재귀적 제너레이터를 조합 반복자라 한다. 파이썬은 네 가지 조합 반복자를 제공한다.
